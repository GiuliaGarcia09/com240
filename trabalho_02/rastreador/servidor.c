/*
  gcc -o exe ./trabalho_02/rastreador/servidor.c -lm && ./exe
  rastreia qual usuário possui os arquivos desejados.
  quando iniciado, oferece 2 opções: 
    1) adiciona os dados do arquivo no repositório
    2) prossegue sem adicionar os dados novos
*/
...
/* struct com nome do arquivo e porta e ip do cliente */
typedef struct{
  char nome_arq[50];
...
/* criando função de adicionar informações no repositório */
int add_arquivo();
...
int main(){
  ...
  /* usa-se a função de adicionar informações no repositório */
  if(i==1){
    add_arquivo();
  }
...
int add_arquivo(){
  ...
  printf("Insera o nome do arquivo: ");
  scanf("%s",inserir.nome_arq);
  fflush(stdin);

  printf("\nInforme a porta do cliente: ");
  ...
  printf("Os dados do arquivo foram inseridos no repositório com sucesso.\n");
  return 0;
}